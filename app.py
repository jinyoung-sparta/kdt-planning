    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if "template_table" in st.session_state and not st.session_state["template_table"].empty:
                ...
                criteria_df = pd.DataFrame(criteria_data)
                criteria_df.to_excel(writer, sheet_name='평가_기준표', index=False)
                ...
                score_df = pd.DataFrame(score_data)
                score_df.to_excel(writer, sheet_name='점수_집계표', index=False)
...
if page == "평가표":
    ...
    st.info("""
        **📋 문제 템플릿 작성 방법**
        1. 평가 템플릿에서 작성한 평가 내용을 바탕으로 문제를 작성
        2. 
    """)
